...
async function sendEthOrDepositERC20(
  ...
  if (nativeToken === ethers.constants.AddressZero) {
    // Send 6 ETH if nativeToken is zero address
    ...
    await tx.wait()
    console.log('6 ETHs are deposited to your account')
  } else {
    ...
    const amount = ethers.utils.parseUnits('6', decimals)
...
export async function ethOrERC20Deposit(
  ...
  await sendEthOrDepositERC20(erc20Inbox, l1Signer)
}
