...
export async function l3Configuration(
  ...
  const provider = new ethers.providers.JsonRpcProvider('https://eth-sepolia.blastapi.io/db024b55-8362-4a77-8764-2419c5838bd6');
  // const latestBlockNumber = await provider.getBlockNumber();

  // // Use the eth_feeHistory method
  // const feeHistory = await provider.send('eth_feeHistory', [1, 'latest', []]);

  // // Extract the base fee from the fee history
  // const baseFeePerGas = ethers.BigNumber.from(feeHistory.baseFeePerGas[0]);

  // console.log('Base Fee Estimate:', ethers.utils.formatUnits(baseFeePerGas, 'gwei'), 'Gwei');

  // const l2Basefee = await L2Provider.getGasPrice()
  // console.log(`L2 Base Fee is ${l2Basefee}`)
  // const totalGasPrice = baseFeePerGas.add(l2Basefee)

  // console.log(`Setting L1 base fee estimate on L3 to ${totalGasPrice}`)

  const tx4 = await ArbOwner.setL1PricePerUnit(0);
  const tx5 = await ArbOwner.setL1PricingRewardRate(0);
  ...
  console.log(
    `L1 rewardRate is set on the block number ${await receipt5.blockNumber} on the Orbit chain`
  )
