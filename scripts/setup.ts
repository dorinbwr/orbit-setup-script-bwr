...
async function main() {
  ...
  console.log(privateKey, L1_RPC_URL, L2_RPC_URL);

  if (!privateKey || !L1_RPC_URL || !L2_RPC_URL) {
  ...
  console.log(L2Provider);
  // Checking if the L1 network is the expected parent chain
  if ((await L1Provider.getNetwork()).chainId !== config.parentChainId) {
    throw new Error(
      'The L1 RPC URL you have provided is not for the correct parent chain'
    )
  }

  // Creating the signer
  const signer = new ethers.Wallet(privateKey).connect(L1Provider)
  ...
  try {
    ...
    if (!rs.etherSent.batchPoster) {
      console.log('Funding batch-poster accounts on parent chain with 0.3 CustomToken')
    ...
    if (!rs.etherSent.staker) {
      console.log('Funding staker accounts on parent chain with 0.3 CustomToken')
    ...
    if (!rs.nativeTokenDeposit) {
      ////////////////////////////////////////////
      /// ETH/Native token deposit to L2 /////////
      ////////////////////////////////////////////
      console.log(
      ...
      const oldBalance = await L2Provider.getBalance(config.chainOwner)
      console.log('Old balance of your account on Orbit chain: ', oldBalance.toString())
      await ethOrERC20Deposit(privateKey, L1_RPC_URL)
      let depositCheckTime = 0

      // Waiting for 30 secs to be sure that ETH/Native token deposited is received on L2
      // Repeatedly check the balance until it changes by 0.4 native tokens
      while (true) {
        depositCheckTime++
        const newBalance = await L2Provider.getBalance(config.chainOwner)
        if (newBalance.sub(oldBalance).gte(ethers.utils.parseEther('6'))) {
          console.log(
        ...
        if (depositCheckTime >= 6) {
          tooLongNotification =
            "(It is taking a long time. Did you change the config files? If you did, you will need to delete ./config/My Arbitrum L2 Chain, since this chain data is for your last config file. If you didn't change the file, please ignore this message.)"
        }
        console.log(
    ...
    if (!rs.tokenBridgeDeployed) {
      ...
      await createERC20Bridge(L1_RPC_URL, privateKey, L2_RPC_URL, config.rollup)
    ...
    if (!rs.l3config) {
      ...
      await l3Configuration(privateKey, L1_RPC_URL, L2_RPC_URL)
      rs.l3config = true
    }
    ////////////////////////////////
    ...
    if (!rs.transferOwnership) {
      ...
      await transferOwner(privateKey, L1Provider, L2Provider)
